...
Script {
    ...
    function exportNotes() {
        ...
        if (numSelectedNotes > 1) {
            var result = script.questionMessageBox(numSelectedNotes + " notes selected.<br/>Export all notes or only selected notes ?", "Export notes", 0x00000800 | 0x00001000, 0x00001000);
            ...
            "children": [
                {
                    ...
                    "type": "folder",
                    "children": (exportAllNotes ? getNoteSubFoldersByParentId(0).concat(getNotesInNoteSubFolder(rootSubFolder)) : getSelectedNotes(selectedNotesIds))
                }
            ]
        };
    ...
    function importItem(item, currentPath) {
        ...
        if (item.type == 'note') {
            ...
            for (var idx in item.children) {
                importItem(item.children[idx], (currentPath === "" ? (item.subject) : (currentPath + "/" + item.subject)));
            }
        }
    }
