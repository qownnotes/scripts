...
jQuery.fn.persianNum = function (options, numberType) {
  $this = jQuery(this);
  persianNumClasses = options
    ? options.persianNumClasses || ["persian"]
    : ["persian"];
  englishNumClasses = options
    ? options.englishNumClasses || ["english"]
    : ["english"];
  arabicNumClasses = options
    ? options.arabicNumClasses || ["arabic"]
    : ["arabic"];
  forbiddenTag = options
    ? options.forbiddenTag || ["SCRIPT", "STYLE", "CODE"]
    : ["SCRIPT", "STYLE", "CODE"];
  for (var j = 0; j < this.length; j++) {
    var el = this[j];
    var className = typeof el.className == "string" ? getClasses(el) : [];
    if (forbiddenTag.indexOf(el.nodeName) >= 0) break;
    for (var i = 0; i < el.childNodes.length; i++) {
      var cnode = el.childNodes[i];
      if (cnode.nodeType == 3) {
        var nval = cnode.nodeValue;
        if (
          hasCommonElements(className, persianNumClasses) ||
          numberType === "p"
        ) {
          cnode.nodeValue = traverse(nval);
        } else if (
          hasCommonElements(className, englishNumClasses) ||
          numberType === "e"
        ) {
          cnode.nodeValue = traverseEn(nval);
        } else if (
          hasCommonElements(className, arabicNumClasses) ||
          numberType === "a"
        ) {
        ...
        if (
          hasCommonElements(getClasses(cnode), persianNumClasses) ||
          numberType === "p"
        ) {
          ...
          jQuery(cnode).persianNum(
            {
              persianNumClasses: persianNumClasses,
              englishNumClasses: englishNumClasses,
              arabicNumClasses: arabicNumClasses,
              forbiddenTag: forbiddenTag,
            },
            "p",
          );
        } else if (
          hasCommonElements(getClasses(cnode), englishNumClasses) ||
          numberType === "e"
        ) {
          if (cnode.nodeName == "OL")
          ...
          hasCommonElements(getClasses(cnode), arabicNumClasses) ||
          numberType === "a"
        ) {
          if (cnode.nodeName == "OL")
...
function getAllClasses(from, until) {
  ...
  jQuery(from)
    ...
    .addBack()
    .each(function () {
      if (this.className) cs.push.apply(cs, this.className.split(" "));
    });
  return cs;
}
